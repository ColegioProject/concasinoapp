...
export default function CoinflipPage() {
  ...
  useEffect(() => {
    if (address) {
      fetch(`/api/leaderboard?limit=1`).catch(() => {})
    }
  }, [address])
  ...
  async function play() {
    ...
    try {
      const res  = await fetch('/api/games/coinflip', {
        ...
        body:    JSON.stringify({ bet, choice, walletAddress: address }),
      })
      const json = await res.json()

      // Let coin flip for 1.5s before showing result
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-6" style={{ background: flash === 'win' ? 'rgba(39,174,96,0.04)' : flash === 'lose' ? 'rgba(231,76,60,0.04)' : 'transparent', transition:'background 0.4s' }}>
      <div className="max-w-lg mx-auto">
        ...
        {/* Back */}
        <Link href="/play" className="font-mono" style={{ fontSize:'10px', color:'var(--gold-dim)', textDecoration:'none', letterSpacing:'2px', display:'inline-block', marginBottom:24 }}>
          ← LOBBY
        </Link>
          ...
          <div className="p-8">
            ...
            <div className="flex justify-center mb-8">
              <div style={{
                ...
                fontSize:40,
                animation: flipping ? 'coinFlip 0.4s linear infinite' : 'none',
                boxShadow: result
                  ? result.outcome === 'win'
            ...
            <div className="flex gap-3 mb-6">
              {(['heads','tails'] as const).map(c => (
                <button key={c} onClick={() => setChoice(c)}
                  ...
                  style={{
                    fontSize:'9px', padding:'12px', cursor:'pointer', letterSpacing:'1px', transition:'all 0.2s',
                    background: choice===c ? 'linear-gradient(135deg,#3a2506,#6b4510,#3a2506)' : 'transparent',
                    border: `1px solid ${choice===c ? 'var(--gold)' : 'var(--gold-dim)'}`,
                    color: choice===c ? 'var(--gold-light)' : 'var(--dim)',
                  }}>
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
              {CHIPS.map(c => (
                <button key={c} onClick={() => setBet(c)}
                  ...
                  style={{
                    fontSize:'8px', padding:'8px 12px', cursor:'pointer', transition:'all 0.15s',
            ...
            {!address ? (
              <div className="text-center">
                <p className="font-mono mb-4" style={{ fontSize:'11px', color:'#b8a070' }}>Connect wallet to play</p>
                <ConnectButton />
              </div>
            ) : (
              <button onClick={play} disabled={loading}
                className="w-full font-pixel shimmer-hover"
                style={{
                  fontSize:'11px', padding:'16px', cursor: loading ? 'wait' : 'pointer', letterSpacing:'2px',
                  background: loading ? '#2a1e08' : 'linear-gradient(135deg,#3a2506,#7a5012,#3a2506)',
                  border:`1px solid ${loading ? 'var(--gold-dim)' : 'var(--gold)'}`,
                  color: loading ? 'var(--gold-dim)' : 'var(--gold-light)',
                  opacity: loading ? 0.7 : 1,
                  boxShadow: loading ? 'none' : '0 0 20px rgba(201,147,58,0.15)',
                }}>
            ...
            {result && !loading && (
              <div className="mt-6 p-4" style={{
                border:`1px solid ${result.outcome==='win' ? 'var(--green)' : 'rgba(231,76,60,0.4)'}`,
                background: result.outcome==='win' ? 'rgba(39,174,96,0.05)' : 'rgba(231,76,60,0.04)',
              }}>
                <div className="font-pixel text-center mb-3" style={{ fontSize:'16px', color: result.outcome==='win' ? 'var(--green)' : '#e74c3c' }}>
                  {result.outcome === 'win' ? '✓ WIN!' : '✗ LOSE'}
                </div>
                <div className="font-mono text-center" style={{ fontSize:'11px', color:'#c8b896' }}>
                  Landed: <strong style={{ color:'var(--gold-light)' }}>{result.resultData.result.toUpperCase()}</strong>
                  {' · '}
                  {result.outcome === 'win'
                    ? <span style={{ color:'var(--green)' }}>+${(result.payout/100).toFixed(2)}</span>
                    : <span style={{ color:'#e74c3c' }}>-${(bet/100).toFixed(2)}</span>
                  }
                </div>
                <div className="font-mono mt-2 text-center" style={{ fontSize:'9px', color:'var(--gold-dim)' }}>
                  VM: {result.vmId} · <a href={`#verify`} style={{ color:'var(--gold-dim)' }}>verify seed</a>
                </div>
              </div>
            )}
