...
const BET_OPTIONS: { type:BetType; label:string; payout:string }[] = [
  { type:'red',    label:'🔴 RED',     payout:'2:1' },
  { type:'black',  label:'⚫ BLACK',   payout:'2:1' },
  { type:'even',   label:'EVEN',       payout:'2:1' },
  { type:'odd',    label:'ODD',        payout:'2:1' },
  { type:'low',    label:'LOW 1-18',   payout:'2:1' },
  { type:'high',   label:'HIGH 19-36', payout:'2:1' },
  { type:'dozen1', label:'1ST 12',     payout:'3:1' },
  { type:'dozen2', label:'2ND 12',     payout:'3:1' },
  { type:'dozen3', label:'3RD 12',     payout:'3:1' },
  { type:'straight', label:'STRAIGHT', payout:'35:1' },
]
...
function RouletteInner() {
  ...
  const numColor = (n: number) => n===0 ? '#27ae60' : RED_NUMS.has(n) ? '#e74c3c' : '#fff'
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="gold-card pixel-corners">
          ...
          <div className="p-8">
            ...
            {betType==='straight' && (
              <div className="mb-5">
                <div className="font-mono mb-2" style={{ fontSize:'9px', color:'var(--gold-dim)', letterSpacing:'2px' }}>NUMBER: <strong style={{ color:'var(--gold-light)' }}>{number}</strong></div>
                <div className="flex flex-wrap gap-1">
                  {Array.from({length:37},(_,i)=>i).map(n => (
                    <button key={n} onClick={() => setNumber(n)} style={{ width:28, height:28, borderRadius:2, fontSize:9, fontWeight:'bold', cursor:'pointer', background: number===n ? numColor(n) : 'transparent', border:`1px solid ${number===n?numColor(n):'rgba(201,147,58,0.2)'}`, color: number===n?'#fff':numColor(n) }}>{n}</button>
                  ))}
                </div>
              </div>
            )}
...
export default function RoulettePage() {
  return <Suspense fallback={<div className="min-h-screen pt-24 flex items-center justify-center font-mono" style={{ color:'var(--gold-dim)' }}>Loading...</div>}><RouletteInner /></Suspense>
}
